...
=======
            ...
            uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(2));
            require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'PolkaBridge AMM V1: K');
>>>>>>> 5ab181f6423c972c19fd97bc5535009f59037aea
        }

        {
            if(amount0In > 0) {
                // _safeTransfer(token0, polkaTreasury, amount0In.div(5000).mul(2));
                _safeTransfer(token0, polkaTreasury, amount0In.div(10000).mul(4));
                // balance0 = balance0 - amount0In.div(5000).mul(2);
                balance0 = balance0 - amount0In.div(10000).mul(4);
            }
            if(amount1In > 0) {
                // _safeTransfer(token1, polkaTreasury, amount1In.div(5000).mul(2));
                _safeTransfer(token1, polkaTreasury, amount1In.div(10000).mul(4));
                // balance1 = balance1 - amount1In.div(5000).mul(2);
                balance1 = balance1 - amount1In.div(10000).mul(4);
            }
        }
